<?php

use Kirby\Cms\App;
use Kirby\Toolkit\Str;

return [
	'areas' => function (App $kirby) {
		$areas = [
			'all'       => 'All pages',
			'guide'     => 'Guide',
			'reference' => 'Reference',
			'cookbook'  => 'Cookbook'
		];

		if ($kirby->option('archived', false) !== true) {
			$areas['kosmos'] = 'Kosmos';
			$areas['plugin'] = 'Plugins';
		}

		return $areas;
	},
	'fields' => [
		'title',
		'keywords',
		'byline' => fn ($page) => strip_tags($page->searchbyline()->kti()),
		'intro'  => function ($page) {
			$html = $page->description()->or($page->intro())->kti();
			return strip_tags($html);
		},
		'text'   => function ($page) {
			return strip_tags($page->text()->kti());
		},
		'area'   => function ($page) {
			if (Str::startsWith($page->id(), 'docs/reference') === true) {
				return 'reference';
			}

			return match ($page->intendedTemplate()->name()) {
				'cookbook-recipe' => 'cookbook',
				'quicktip'        => 'cookbook',
				'guide'           => 'guide',
				'kosmos-issue'    => 'kosmos',
				'plugin'          => 'plugin',
				default           => null
			};
		},
		'weight'  => function ($page) {
			return match ($page->intendedTemplate()->name()) {
				'guide',
				'cookbook-recipe'     => 2,
				'reference-classmethod',
				'reference-component',
				'reference-endpoint',
				'reference-fieldmethod',
				'reference-helper',
				'reference-hook',
				'reference-kirbytag',
				'reference-validator' => 0.5,
				'referece-icon',
				'reference-root',
				'reference-url'       => 0.25,
				default               => 1
			};
		},
		'template' => fn ($page) => $page->intendedTemplate()->name()
	],
	'templates' => [
		'cookbook-category',
		'cookbook-recipe',
		'glossary',
		'guide',
		'kosmos-issue',
		'plugin',
		'quicktip',
		'reference-article',
		'reference-block',
		'reference-class',
		'reference-classes',
		'reference-classmethod',
		'reference-component',
		'reference-endpoint',
		'reference-endpoints',
		'reference-extension',
		'reference-extensions',
		'reference-fieldmethod',
		'reference-fieldmethods',
		'reference-helper',
		'reference-helpers',
		'reference-hook',
		'reference-hooks',
		'reference-icon',
		'reference-icons',
		'reference-kirbytag',
		'reference-kirbytags',
		'reference-panelsection',
		'reference-panelsections',
		'reference-root',
		'reference-roots',
		'reference-section',
		'reference-ui',
		'reference-url',
		'reference-validator',
		'reference-validators',
		'security',
		'text',
		'release',
		'release-35',
		'release-4',
		'release-5'
	],
	'providers' => [
		'algolia' => [
			'app'   => 'S7OGBIAJTV',
			'index' => 'getkirby-4',
		],
		'loupe' => [
			'searchable' => ['title', 'intro', 'text', 'keywords', 'byline'],
			'filterable' => ['area', 'template', 'weight'],
			'sortable'   => ['title', 'weight'],
		]
	]
];
